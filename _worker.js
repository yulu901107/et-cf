...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, vlessResponseHeader, log,) {
    ...
    const tcpSocket = enableSocks ? await connectAndWrite(addressRemote, portRemote, true) : await connectAndWrite(proxyIP || addressRemote, portRemote);
...
const v2ray_protocol_type = 'vless';    // 'dmxlc3M=';
function gen_v2ray_config(UUID, server_address) {
    // const 协议类型 = v2ray_protocol_type;
    
    const v2ray_remarks = server_address;
    let v2ray_address = server_address;
    let v2ray_port = 443;

    const v2ray_uuid = UUID;
    const v2ray_encryption = 'none';
    
    const v2ray_transport_network = 'ws';
    const v2ray_transport_host = server_address;
    const v2ray_transport_path = '/?ed=2560';
    
    let v2ray_tls = ['tls',true];
    const v2ray_sni = server_address;
    const v2ray_fingerprint = 'randomized';

    if (server_address.includes('.workers.dev')){
        v2ray_address = 'www.wto.org';
        v2ray_port = 80 ;
        v2ray_tls = ['',false];
    }

    const v2ray = `${v2ray_protocol_type}://${v2ray_uuid}\u0040${v2ray_address}:${v2ray_port}?encryption=${v2ray_encryption}&security=${v2ray_tls[0]}&sni=${v2ray_sni}&fp=${v2ray_fingerprint}&type=${v2ray_transport_network}&host=${v2ray_transport_host}&path=${encodeURIComponent(v2ray_transport_path)}#${encodeURIComponent(v2ray_remarks)}`;
    const clash = `- type: ${v2ray_protocol_type}
  name: ${v2ray_remarks}
  server: ${v2ray_address}
  port: ${v2ray_port}
  uuid: ${v2ray_uuid}
  network: ${v2ray_transport_network}
  tls: ${v2ray_tls[1]}
  udp: false
  sni: ${v2ray_sni}
  client-fingerprint: ${v2ray_fingerprint}
  ws-opts:
    path: "${v2ray_transport_path}"
    headers:
...
async function getVLESSConfig(userID, hostName, sub, UA, RproxyIP, _url) {
    ...
    if ( userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        let subscriber_msg = `您的订阅内容由 ${sub} 提供维护支持, 自动获取ProxyIP: ${RproxyIP}`;
        if (!sub || sub == '') {
            if (!proxyIP || proxyIP =='') {
                subscriber_msg = '您的订阅内容由 内置 addresses/ADD 参数提供, 当前使用的ProxyIP为空, 推荐您设置 proxyIP/PROXYIP ！！！';
            } else {
                subscriber_msg = `您的订阅内容由 内置 addresses/ADD 参数提供, 当前使用的ProxyIP： ${proxyIPs.join(',')}`;
            }
        } else if (RproxyIP != 'true'){
            subscriber_msg += `, 当前使用的ProxyIP： ${proxyIPs.join(',')}`;
        }
        return `
################################################################
Subscribe / sub 订阅地址, 支持 Base64、clash-meta、sing-box 订阅格式, ${subscriber_msg}
---------------------------------------------------------------
快速自适应订阅地址:
...
`;
        ...
        // 如果是使用默认域名，则改成一个workers的域名，subscriber_msg会加上代理
...
function subAddresses(host,UUID,noTLS,newAddressesapi,newAddressescsv,newAddressesnotlsapi,newAddressesnotlscsv) {
    ...
    const responseBody = uniqueAddresses.map(address => {
        ...
        // const 协议类型 = v2ray_protocol_type;
        const vlessLink = `${v2ray_protocol_type}://${UUID}\u0040${address}:${port}?encryption=none&security=tls&sni=${v2ray_transport_host}&fp=random&type=ws&host=${v2ray_transport_host}&path=${encodeURIComponent(v2ray_transport_path_final)}#${encodeURIComponent(addressid + v2ray_node_comment)}`;
